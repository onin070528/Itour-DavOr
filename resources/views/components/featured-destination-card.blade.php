...
<article class="group flex flex-col overflow-hidden rounded-md border border-sand-200 bg-sand-0 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
    <div class="relative flex h-44 items-end overflow-hidden bg-sand-200">
        <img
            src="{{ asset('storage/itour-images/'.$destination['image']) }}"
            alt="{{ $destination['name'] }}, {{ $destination['municipality'] }}"
            loading="lazy"
        ...
        <span class="relative m-3 rounded-sm bg-sand-900/45 px-2.5 py-1 text-xs font-semibold text-sand-0">{{ $destination['municipality'] }}</span>
    </div>
