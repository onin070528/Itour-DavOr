...
{{-- One +/- counter block, wired up generically in initEstablishmentQrForm()
     (resources/js/app.js) via the data-counter attribute. `compact` drops
     the card/label wrapper for use inside a table cell whose row already
     carries the label (see the Companion Headcount matrix below). --}}
@if ($compact)
    <div data-counter="{{ $name }}" class="flex items-center justify-center gap-1.5">
        <button
            type="button"
        ...
        <span data-counter-value class="flex h-7 min-w-8 items-center justify-center rounded-sm border border-sand-300 bg-sand-0 text-sm font-bold text-sand-900">0</span>
        <button
            type="button"
