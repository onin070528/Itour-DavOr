@php
    $destinationListings = collect($destinations)->where('category', 'destinations')->values();
@endphp
...
<x-layouts.dashboard :user="$user" :nav-sections="$navSections" :page-title="$pageTitle" account-heading="System" :settings-href="route('pto.settings')">
    <x-dashboard.page-header
        title="Destinations"
        description="Manage tourism destinations across the province's 11 municipalities."
    >
        <x-slot:actions>
            <button type="button" data-modal-open="destination-form-modal" data-modal-mode="add" class="inline-flex items-center gap-2 rounded-sm bg-primary-700 px-4 py-2.5 text-sm font-semibold text-sand-0 hover:bg-primary-900">
    ...
    <div data-filterable-table class="mt-6">
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($destinationListings as $d)
                @php
                ...
                @endphp
                <div data-row data-municipality="{{ $d['municipality'] }}" data-search-text="{{ strtolower($d['name'].' '.$d['municipality']) }}" class="overflow-hidden rounded-md border border-sand-200 bg-sand-0">
                    <div class="relative h-32 bg-sand-200">
                        ...
                        <span class="absolute top-2 left-2 rounded-sm bg-sand-900/50 px-2 py-0.5 text-[11px] font-semibold text-sand-0">{{ $d['municipality'] }}</span>
                    </div>
                    <div class="p-3.5">
                            ...
                            <div class="relative shrink-0">
                                ...
                                <div data-dropdown-menu class="absolute right-0 z-10 mt-1 hidden w-40 rounded-md border border-sand-200 bg-sand-0 py-1 shadow-md">
                                    ...
                                    <button
                                        ...
                                        data-edit-action="{{ route('pto.directory.destinations.update', $d['id']) }}"
                                    ...
                                    <form method="POST" action="{{ route('pto.directory.destinations.archive', $d['id']) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button
                                            type="button"
                        ...
                        <p class="mt-1 flex items-center gap-1 text-xs text-sand-500"><i class="ti ti-map-pin" aria-hidden="true"></i>{{ $d['barangay'] }}</p>
                ...
                {{-- Per-destination read-only details modal --}}
                <x-dashboard.modal id="destination-view-{{ $d['id'] }}" :title="$d['name']">
                    ...
                    <dl class="flex flex-col gap-3 text-sm">
                        <div><dt class="text-xs font-semibold text-sand-500 uppercase">Municipality</dt><dd class="text-sand-800">{{ $d['municipality'] }}</dd></div>
    ...
    <x-dashboard.modal id="destination-form-modal" title="Destination">
        <form
            id="destination-form"
            method="POST"
            action="{{ route('pto.directory.destinations.store') }}"
            data-default-action="{{ route('pto.directory.destinations.store') }}"
        ...
        >
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input name="contactOffice" type="text" class="w-full rounded-sm border border-sand-300 px-3 py-2 text-sm">
                </div>
                <div>
